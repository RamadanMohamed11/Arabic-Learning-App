...
وقال في نفسه: بترشيد الكهرباء نحافظ على مواردنا ونساعد وطننا.''';
  ...
  void _checkAnswers() {
    ...
    if (correct == questions.length) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'حصلت على $_score من ${questions.length}. بعض الإجابات تحتاج مراجعة!',
          ),
        ...
        child: SafeArea(
          child: ListView(
            ...
            children: [
              ...
              ...List.generate(
                questions.length,
                (i) => _buildQuestionCard(i, questions[i]),
              ),
              const SizedBox(height: 24),
  ...
  Widget _buildReadingModeText() {
    final allDone =
        _sentenceResults.length == _sentences.length &&
        ...
        children: [
          ...
          ...List.generate(q['options'].length, (optIndex) {
            ...
            return GestureDetector(
              onTap: () {
                if (!_isCompleted)
                  setState(() => _selectedAnswers[questionIndex] = optIndex);
              },
              child: Container(
                margin: const EdgeInsets.only(bottom: 10),
                padding: const EdgeInsets.symmetric(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: getStatusColor(),
                    width:
                        isSelected || (showCorrectIncorrect && isCorrectOption)
                        ? 2
                        : 1,
                  ),
                ),
